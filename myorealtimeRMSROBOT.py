...
model1 = joblib.load('jempolnaufalrmsPCA.pkl')
model2 = joblib.load('telunjuknaufalrmsPCA.pkl')
model3 = joblib.load('tengahnaufalrmsPCA.pkl')
model4 = joblib.load('manisnaufalrmsPCA.pkl')
model5 = joblib.load('kelingkingnaufalrmsPCA.pkl')
    ...
    def main(self):
        ...
        while True :
            ...
            if oi[0] > 39:
                ...
                #print(datafituran)
                #X1 = modelscaler.transform(datafitur)
                ...
                f = np.array(realtime1)
                g = np.array(realtime2)
                h = np.array(realtime3)
                i = np.array(realtime4)
                j = np.array(realtime5)
                

 
                
                datajadi = np.hstack((e,f,g,h,i)).tolist()
                 

                #datahasil.append(datajadi)
            #datahasil1 = pd.DataFrame(np.array(datahasil).reshape(-1,5))
            #datahasil1.to_csv("datahasilRMSke000PCAbenerfix.csv")
                ...
                arduino.flush()
                time.sleep(1)
                arduino.write(struct.pack('>BBBBB',a,b,c,d,e))
                ...
                #wesurutan1 = np.array(wesurutan)dg
                #print(wesurutan, wesurutan.shape)
